...
public class ScannerTests {
	...
	public void testNoSpace() throws Exception {
		...
		token = scanner.nextToken();
		assertEquals(FLOAT_LIT, token.kind);
		assertEquals(13.55, token.getFloatValue(), 0.001);
		token = scanner.nextToken();
		assertEquals(PLUS, token.kind);
		token = scanner.nextToken();
		assertEquals(EOF, token.kind);
	...
	@Test
	public void testSingleCharacterNoSpace() throws Exception {
		String input = "+*/%&|,.(){}";
		Scanner scanner = new Scanner(input);
		scanner.scan();
		Token token = scanner.nextToken();
	...
	@Test
	public void testMultipleCharacterNoSpace() throws Exception {
		String input = "=<>!-==<=>=!=->";
		Scanner scanner = new Scanner(input);
		scanner.scan();
		Token token = scanner.nextToken();
